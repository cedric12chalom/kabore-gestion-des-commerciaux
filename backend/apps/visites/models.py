...
class Visite(models.Model):
    """Visite de contrôle d'un point de vente, effectuée par un manager."""
    ...
    class Type(models.TextChoices):
        CONTROLE_PDV = 'CONTROLE_PDV', _('Contrôle point de vente')
    ...
    point_vente_nom = models.CharField(_('point de vente'), max_length=200, default='')
    ...
    type_visite = models.CharField(
        ...
        choices=Type.choices, default=Type.CONTROLE_PDV,
    )
    ...
    statut = models.CharField(
        ...
        choices=Statut.choices, default=Statut.PLANIFIEE, db_index=True,
    )

    checkin_position = gis_models.PointField(
        _('position check-in'), srid=4326, geography=True, null=True, blank=True,
    )
    checkin_timestamp = models.DateTimeField(_('heure check-in'), null=True, blank=True)
    checkout_position = gis_models.PointField(
        _('position check-out'), srid=4326, geography=True, null=True, blank=True,
    )
    ...
    note_controle = models.PositiveSmallIntegerField(
        _('note de contrôle'),
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=['manager', 'statut', 'date_prevue']),
            models.Index(fields=['point_vente_nom', 'date_prevue']),
            models.Index(fields=['statut', 'date_prevue']),
        ]

    def __str__(self):
        return f"Visite {self.point_vente_nom} - {self.date_prevue.strftime('%d/%m/%Y')}"
...
class RapportVisite(models.Model):
    ...
    constats = models.TextField(_('constats'), blank=True)
    ...
    signature = models.TextField(_('signature électronique'), blank=True)
